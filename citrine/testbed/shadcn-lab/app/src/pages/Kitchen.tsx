// 组件走查页：shadcn/ui 核心组件的静息 / 选中 / 禁用 / 出错状态铺在一页，配方层公共类同页对照。
// 浮层触发器带 data-ks-open（走查脚本逐个点开扫描），弹层触发器带 data-ks-modal。每个 <section class="ks" data-ks="…"> 是一组。
    ...
    <>
      ...
      <Sec id="badges" title="徽标与状态胶囊">
        <div className="row">
          ...
          <span className="status success">已完成</span><span className="status warning">待接单</span><span className="status error">已取消</span><span className="status info">配送中</span><span className="status neutral">草稿</span><span className="badge brand">管理员</span>
        </div>
      </Sec>

      <Sec id="inputs" title="输入">
        <div className="grid2">
          <div className="field"><Label htmlFor="k-name">名称</Label><Input id="k-name" placeholder="请输入商户名称" /></div>
          <div className="field"><Label htmlFor="k-search">搜索</Label><div className="with-icon"><Search className="i-icon--sm i-icon--muted" aria-hidden="true" /><Input id="k-search" placeholder="订单号 / 商户 / 骑手" /></div></div>
      ...
      <Sec id="select" title="下拉选择">
        <div className="row">
          <Select defaultValue="all">
            <SelectTrigger className="w-44" aria-label="订单状态" data-ks-open data-ks-type="select"><SelectValue /></SelectTrigger>
            <SelectContent><SelectItem value="all">全部状态</SelectItem><SelectItem value="pending">待接单</SelectItem><SelectItem value="done">已完成</SelectItem><SelectItem value="cancel" disabled>已取消</SelectItem></SelectContent>
          </Select>
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button variant="outline" data-ks-open data-ks-type="dropdown">更多操作</Button></DropdownMenuTrigger>
            <DropdownMenuContent><DropdownMenuLabel>订单</DropdownMenuLabel><DropdownMenuItem>改派</DropdownMenuItem><DropdownMenuItem>催单</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive">取消订单</DropdownMenuItem></DropdownMenuContent>
          </DropdownMenu>
          <Popover>
      ...
      <Sec id="table" title="表格（shadcn Table + 配方层胶囊）">
        <Table>
          <TableHeader><TableRow><TableHead>订单号</TableHead><TableHead>商户</TableHead><TableHead className="text-right">金额</TableHead><TableHead>状态</TableHead><TableHead>操作</TableHead></TableRow></TableHeader>
          <TableBody>
            {[['20260908-0412', '鲜果时光', '¥156.70', 'success', '已完成'], ['20260908-0411', '川味小馆', '¥47.80', 'warning', '待接单'], ['20260908-0410', '晨曦烘焙', '¥1,225.90', 'error', '已取消']].map(([no, m, amt, tone, st]) => (
              <TableRow key={no}><TableCell><a className="link mono" href="#/orders">{no}</a></TableCell><TableCell>{m}</TableCell><TableCell className="text-right num">{amt}</TableCell><TableCell><span className={`status ${tone}`}>{st}</span></TableCell><TableCell><a className="act" href="#/orders">详情</a><button className="act" type="button">改派</button><button className="act danger" type="button">取消</button></TableCell></TableRow>
            ))}
          </TableBody>
        </Table>
      </Sec>
      ...
      <Sec id="overlays" title="弹窗 / 确认">
        <div className="row">
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader><DialogTitle>改派订单</DialogTitle><DialogDescription>选择一名骑手接手该订单。</DialogDescription></DialogHeader>
              <div className="field"><Label htmlFor="d-rider">骑手</Label><Input id="d-rider" placeholder="搜索骑手" /></div>
              <DialogFooter><DialogClose asChild><Button variant="outline">取消</Button></DialogClose><Button>确认改派</Button></DialogFooter>
            </DialogContent>
          </Dialog>
          <AlertDialog>
            <AlertDialogTrigger asChild><Button variant="destructive" data-ks-modal="alert">删除商户</Button></AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader><AlertDialogTitle>确认删除「川味小馆」？</AlertDialogTitle><AlertDialogDescription>删除后不可恢复，历史订单仍会保留。</AlertDialogDescription></AlertDialogHeader>
      ...
      <Sec id="feedback" title="提示 / 进度 / 骨架">
        <div className="stack">
          <Alert><Info /><AlertTitle>今日有 7 个商户资质即将过期</AlertTitle><AlertDescription>请在到期前完成复审。</AlertDescription></Alert>
      ...
      <Sec id="cards" title="卡片与统计卡">
        ...
        <div className="row">
          <span className="chip">状态：待接单 <button type="button" className="x" aria-label="移除条件 状态">×</button></span>
