...
| D69 | 操作链接需要更明确的可点击感；「查看全部」是另一类（用户规则） | 纯文字 500 字重的动作与正文区分仍弱；导览入口与动作混在一类 | 规则 + 公共类 | 2.1.2：操作链接同色胶囊描边（`.act`），导览链接文字 + 箭头（`.go`）；链接分四层 |
| D68 | 动作链接（编辑 / 复制 / 详情…）套了内容型链接的斜体下划线（用户反馈） | 一套链接样式承担了内容与动作两种语义 | 规则 + 公共类 | 2.1.1：链接分三层，新增 `.act` 操作链接；25 处改用 |
| D67 | 暗色人眼走查 | 整体一致；已结束进度条灰与轨道几乎同色；登录页暗色未被验收覆盖；"已读行更亮"为截图伪影 | token + 工具 | 2.1.0：新增 `data.inactive`；页面清单去掉 login 的强制亮色 |
| D66 | 用户决定移除紧凑模式 | 从未被真正需要，却让每条规则多验一种模式 | 主题（major） | 2.0.0：删除 compact Theme 及全部开关、参数、文档表述 |
| D65 | 数据卡左描边应是固定特征而非高亮（用户规则） | 只有一张卡带描边 | 组件 + 配方 | 1.3.3：所有 StatCard 默认左描边，预览同步 |
| D64 | 工作台与看板的统计卡样式各写一份（用户要求同步） | 复制粘贴的样式已漂移（高亮描边只在一页） | 项目组件 + 骨架配方 | 1.3.2：`StatCard.vue` 共用，规则"统计卡封装复用" |
| D63 | 视觉自审（人眼层）：17 页 × 亮暗 | 黄底头像、纯黄角色徽标、危险链接同色、保存草稿档位、12 张卡全黄进度条、订单号斜体、单数字统计卡拉满、详情页间距 | 配方 + 公共类 + 桥接 | 1.3.1：8 条全部按建议改到系统层（用户批准） |
