...
| D70 | 盲测三：详情 + 页签 + 操作日志 | 新规则全部被正确使用；13 条猜测集中在详情页骨架、页头动作档位、胶囊禁用态、中性时间线、内容级不存在、附件卡 | 文档 + 公共类 | 2.1.3：详情页骨架配方、`.act` 禁用与 button 重置进公共类、附件卡配方；两处文档矛盾修正 |
| D71 | 暗色深底整族去蓝（用户反馈"板块和背景过蓝"） | graphite 色相对齐 gray 族暗端后 C≈0.031，在大面积深底上被放大成藏青；只需降色度，明度与色相不动即可保住冷调与全部对比度 | token（graphite 7 档 + scrim / mask + 5 个 wash 重算） | 2.2.0：OKLCH C 压到 60%（≈0.018）；亮色 neutral.900 与旧 graphite.850 同值属巧合、不动；预览 `.dark` 块按此区分同步，验收全绿、E/S 一致 |
| D72 | 操作胶囊加描边后"太大太重"，且 `<a>` / `<button>` 粗细不一 | 胶囊沿用按钮最小档 sm（28）与 500 字重，和同一行 24 高、400 字重的状态胶囊放一起就显重；`button.act { font: inherit }` 简写把字重行高一并重置 | token（新增 `control.height.xs`）+ 公共类 + 文档 | 2.3.0：`.act` 改 24 高 / 8 内边距 / 13px / 400，字体三项由 `.act` 自己声明；DESIGN 控件档位与操作列宽估算同步 |

D73 起的发现来自实测项目三「轻采」的接入试点，见 [`../light-procure/FINDINGS.md`](../light-procure/FINDINGS.md)。
