...
// 用法：node tools/check-focus.mjs [--page orders|kitchen|...] [--steps 80]   退出码：出现 NO RING 为 1。
import { launch, sleep } from './lib/cdp.mjs';
import { serve } from './lib/serve.mjs';
import { APP_DIST, requireDist, args } from './lib/paths.mjs';

requireDist();
const opt = args();
const pages = String(opt.page || 'orders,apply').split(',');
const steps = Number(opt.steps || 80);
...
try {
  for (const page of pages) {
    ...
    await browser.goto(`${server.url}/index.html?role=admin#/${page === 'dashboard' ? '' : page === 'apply' ? 'merchants/apply' : page}`); await sleep(1500);
