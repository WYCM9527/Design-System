...
import { APP_DIST, TOKENS_DIST, requireDist, outDir, args, parseModes, modeQuery } from './lib/paths.mjs';
...
try {
  for (const mode of modes) {
    ...
    await browser.goto(`${server.url}/index.html?role=admin&${modeQuery(mode)}#/kitchen`, "document.readyState === 'complete' && document.querySelectorAll('.ks').length >= 10");
    await sleep(1500);
