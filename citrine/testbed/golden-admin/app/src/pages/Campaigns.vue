...
<template>
  ...
  <el-card v-if="view === 'list'" class="flat" shadow="always">
    <el-table :data="campaigns" style="width: 100%">
      ...
      <el-table-column label="操作" width="170" fixed="right"><template #default="{ row }"><a class="link">编辑</a><template v-if="can('campaign.edit') && ['进行中', '待审核', '已下线'].includes(row.status)"> · <a class="link" @click="toggle(row)">{{ row.status === '进行中' ? '下线' : '上线' }}</a></template> · <a class="link">复制</a></template></el-table-column>
    </el-table>
  </el-card>
