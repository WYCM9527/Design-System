...
4. 按钮不能裸放：有描边（次要）或浅底（quiet 文字按钮）。链接分三层：内容型（订单号、名称：下划线 + 斜体）、操作型（编辑 / 详情 / 改派等动词：500 字重、不斜体不下划线、hover 出下划线、危险红字）、标题型（列表项标题：近黑 + 500）。表格操作列默认操作链接串，审批类决定才用 quiet 按钮。
...
| 操作链接（编辑 / 复制 / 详情 / 改派 / 催单 / 取消选择…） | — | `text.link` + `text.weight.label`，hover `text.link-hover` + 下划线；危险动作 `text.danger`（hover 不变色） | 不斜体、常态无下划线；多个动作用「·」分隔，一行最多 3 个，其余收进「⋯」；命中区 ≥ `control.hit-min` |
