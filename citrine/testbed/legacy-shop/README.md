...
| `migrate --phase replace` | 只有 1 处无歧义（顶栏 56px → `layout.topbar.height`）；45 处待决：`#FFFFFF` 在多个 bg 语义间歧义，`14px` 同时匹配 `text.body.size` / `icon.size.sm` / `table.cell.padding-y`（匹配不看属性——skill 缺陷，已在 steward 0.6.0 修正：按 CSS 属性先筛同类 Token） |
...
3. 尺寸匹配不看 CSS 属性（`font-size: 14px` 也给出 `icon.size.sm`）会放大待决清单——steward 0.6.0 起按属性先筛同类 Token，`font-size` 只在字号 / 图标尺寸里找；同批修正的还有注释里的值不再计数、`settle --apply --decisions-file` 可直接落地归并决定、`--allow-dirty` 生效、guard / status 输出各自的职责边界。
