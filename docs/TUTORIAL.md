...
接入完成后，规则只是被 AI"看见"——它改完页面是否记得跑验收，取决于它听不听话。把验收放进 CI（项目托管在 GitHub 时），每次提交 / 合并请求都会自动跑一遍，不合格的改动进不了主分支。这是**唯一不依赖 AI 自觉**的保险，做一次就行：

```text
把设计系统验收接进 CI：按 design-system-adopter 用 ds.mjs ci --dry-run 把要生成的工作流文件给我看，用大白话说明它会在什么时候跑、检查什么；我确认后写入 .github/workflows/ 并提交。如果项目还没有 accept.config.mjs，先按模板建一份再做。
```

之后在 GitHub 的 Pull Request 页面能看到一个叫 **Design System** 的检查项：绿的就是合格；红的点进去看哪一项没过，把失败信息粘给 AI 修。项目不在 GitHub（GitLab 等）也能做，让 AI 照工作流里的步骤改写成你们平台的格式。
