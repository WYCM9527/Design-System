...
9. **验收进 CI**：`ds.mjs ci` 写出 `.github/workflows/design-system.yml`（先看 `--dry-run` 给用户），提交后每次 PR / 推 main 自动跑 guard + 验收——日常做页面时 Agent 是否记得跑验收不再是唯一防线。GitLab 等照工作流里的 shell 步骤搬。
