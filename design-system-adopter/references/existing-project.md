...
- 旧项目用的组件库没有桥接（不在身份文件 `stacks` 里，如 Ant Design / Vuetify / Naive / 纯手写）：**在第 0 步就如实告知**——第一期换肤只能覆盖页面骨架与 token，组件仍是旧样子；正确路径是把项目路径交给设计系统维护者，评估后为该组件库新写一份桥接（系统层工作，按既有 Element 桥接的做法逐组件走查，约 2–3 天，写完同栈项目复用）。不要现场给页面糊样式冒充桥接，也不要自己在项目里造一份桥接副本。
